(function() {
  ...
  function initSecurityHub() {
    ...
    var PLAYZONE_SECURITY_AUDIT = [
      ...
      { id: 'PZ-005', priority: 5, severity: 'critical', necessity: 'inmediata', category: 'auth', title: 'creatorUid no ligado a auth.uid', description: 'El .validate de missions/$missionId solo exige hasChildren(game, title, creatorUid), sin comprobar que creatorUid sea el del autor.', impact: 'Suplantar a otro jugador como anfitrión y heredar sus permisos de líder (status, startedAt, expulsar, inscribir).', location: 'database.rules.json → missions/$missionId (327-328)', remediation: 'Añadir al .validate newData.child(\'creatorUid\').val() === auth.uid e impedir su modificación posterior.' },
      { id: 'PZ-006', priority: 6, severity: 'critical', necessity: 'inmediata', category: 'auth', title: 'Clave de API de Steam en el repositorio', description: 'STEAM_API_KEY está escrita en texto plano dentro del código de la Cloud Function de verificación CS2.', impact: 'Cualquiera con acceso al repo usa la clave, agota la cuota Steam y tumba la verificación de misiones CS2.', location: 'functions/cs2FriendsMission.js (19)', remediation: 'Rotar la clave ya expuesta y moverla a Secret Manager o functions.config().' },
      { id: 'PZ-007', priority: 7, severity: 'critical', necessity: 'inmediata', category: 'economy', title: 'sponsoredReward y boost falsificables al crear', description: 'sponsoredReward, commanderRewardBoost y commanderRewardBoostBy tienen .write de commander pero su .validate solo comprueba el tipo, y el .write del padre permite escribirlos en la creación.', impact: 'Marcar la misión como patrocinada y con boost de Commander sin ningún rango, multiplicando el premio.', location: 'database.rules.json → missions/$missionId (376-391)', remediation: 'Validar en la creación que estos campos no existan salvo que el autor tenga rango Commander o Boss.' },
      { id: 'PZ-008', priority: 8, severity: 'high', necessity: 'alta', category: 'xss', title: 'XSS en tarjeta de misión (título, juego, avatar)', description: 'renderMissionCard escapa la descripción con escHtml pero interpola cardTitle, mission.game, skill, type y el avatar del participante directamente en innerHTML.', impact: 'Una misión creada con título malicioso ejecuta script en el navegador de cualquiera que abra Play Zone.', location: 'playzone.js → renderMissionCard (1549-1592)', remediation: 'Pasar todos esos campos por escHtml y validar que avatar sea una URL https permitida.' },
      { id: 'PZ-009', priority: 9, severity: 'high', necessity: 'alta', category: 'xss', title: 'XSS en tarjeta de jugador y onclick inline', description: 'renderPlayerCard inserta user.nick sin escapar y construye onclick="inviteToMission(\'uid\', \'nick\')" escapando solo la comilla simple.', impact: 'Un nick con comillas dobles o etiquetas rompe el atributo y ejecuta script al abrir la pestaña Jugadores.', location: 'playzone.js → renderPlayerCard (1814-1842)', remediation: 'Escapar HTML y atributos, y sustituir los onclick inline por addEventListener con data-*.' },
      { id: 'PZ-010', priority: 10, severity: 'high', necessity: 'alta', category: 'missions', title: 'Cupos de misión solo validados en la interfaz', description: 'joinMission hace un set directo en participants y las reglas no cuentan participantes frente a maxParticipants.', impact: 'Saltarse el botón "Misión llena" desde consola y meter cuentas ilimitadas para forzar confirmaciones y premios.', location: 'playzone.js → joinMission (1696-1731) + database.rules.json → participants (329-332)', remediation: 'Unirse mediante transacción o Cloud Function que cuente participantes y rechace si se supera el cupo.' },
      { id: 'PZ-011', priority: 11, severity: 'high', necessity: 'alta', category: 'missions', title: 'El anfitrión puede inscribir a terceros', description: 'La regla de participants/$participantUid permite escribir tanto al propio jugador como al creatorUid de la misión.', impact: 'Meter jugadores sin su consentimiento, inflar cupos y montar misiones fantasma para farmear tokens.', location: 'database.rules.json → missions/$missionId/participants/$participantUid (329-332)', remediation: 'Permitir alta solo con auth.uid === $participantUid y dejar al creador únicamente la expulsión (remove).' },
      { id: 'PZ-012', priority: 12, severity: 'high', necessity: 'alta', category: 'missions', title: 'Invitaciones con remitente falsificable', description: 'missionInvites/$targetUid/$missionId tiene .write auth != null y su .validate no comprueba que fromUid coincida con auth.uid.', impact: 'Spam y phishing de invitaciones suplantando la identidad de cualquier jugador.', location: 'database.rules.json → missionInvites (551-558)', remediation: 'Validar fromUid === auth.uid y limitar la frecuencia de envío por emisor.' },
      { id: 'PZ-013', priority: 13, severity: 'high', necessity: 'alta', category: 'missions', title: 'nexusVerifiedComplete escribible por participantes', description: 'La regla permite que cualquier participante selle nexusVerifiedComplete sin que el servidor haya validado nada.', impact: 'Marcar la misión como verificada por el Nexo y ensuciar historial y estadísticas de verificación.', location: 'database.rules.json → missions/$missionId/nexusVerifiedComplete (363-365)', remediation: 'Poner .write en false y dejar el sellado exclusivamente a awardMissionTokens.' },
      { id: 'PZ-014', priority: 14, severity: 'high', necessity: 'alta', category: 'data', title: 'Historial de partidas verificadas falsificable', description: 'users/$uid/extraVerifiedMatches permite escritura al propio usuario con un .validate que solo exige at, title y type.', impact: 'Inventar partidas verificadas en el perfil sin haber completado ninguna misión.', location: 'database.rules.json → users/$uid/extraVerifiedMatches (190-194)', remediation: 'Poner .write en false para el cliente y escribir estas entradas solo desde Cloud Functions.' },
      { id: 'PZ-015', priority: 15, severity: 'high', necessity: 'alta', category: 'xss', title: 'XSS en chat de misión vía photoURL', description: 'renderHubChatMessage escapa nick y texto con pzEsc pero coloca msg.photoURL sin filtrar dentro del atributo src de la imagen.', impact: 'Un participante ejecuta script en el navegador del resto del equipo desde el chat del hub.', location: 'playzone.js → renderHubChatMessage (3248-3263)', remediation: 'Aceptar solo URLs https de dominios permitidos o derivar el avatar del UID en el cliente.' },
      { id: 'PZ-016', priority: 16, severity: 'high', necessity: 'alta', category: 'xss', title: 'XSS en chat privado vía photoURL e imageUrl', description: 'renderPrivateChatMessage escapa nick y texto pero interpola photoURL e imageUrl crudos en atributos src.', impact: 'Ejecución de script entre usuarios que abren un chat 1 a 1 desde Play Zone.', location: 'playzone.js → renderPrivateChatMessage (3476-3494)', remediation: 'Validar el esquema de las URLs y servir imágenes solo desde Storage con reglas estrictas.' },
      { id: 'PZ-017', priority: 17, severity: 'high', necessity: 'alta', category: 'privacy', title: 'Descarga masiva del nodo users', description: 'users tiene .read auth != null y Play Zone carga y filtra usuarios en el cliente para la búsqueda y la pestaña Jugadores.', impact: 'Cualquier autenticado extrae el padrón completo: nicks, rangos, Steam y metadatos de todos los jugadores.', location: 'database.rules.json → users (.read) + playzone.js → loadPlayers / searchHubUsersForInvite', remediation: 'Cerrar la lectura global y exponer un índice público mínimo o una Callable con límite de peticiones.' },
      { id: 'PZ-018', priority: 18, severity: 'medium', necessity: 'alta', category: 'missions', title: 'Chat de misión sin esquema en reglas', description: 'missions/$missionId/chat y missionChats/$missionId permiten escribir a los participantes sin ningún .validate de campos, autoría ni longitud.', impact: 'Suplantar el senderUid, inyectar photoURL malicioso (ver PZ-015) y subir cargas enormes a la base.', location: 'database.rules.json → missions/$missionId/chat (342-345) y missionChats (574-581)', remediation: 'Añadir .validate exigiendo senderUid === auth.uid, texto acotado y URLs restringidas.' },
      { id: 'PZ-019', priority: 19, severity: 'medium', necessity: 'media', category: 'economy', title: 'tokensAwarded inyectable en la creación', description: 'Nada impide incluir tokensAwarded true en el objeto inicial de la misión, y awardMissionTokens aborta en cuanto lo detecta por idempotencia.', impact: 'Sabotear misiones legítimas: el equipo completa la misión y nunca cobra el premio.', location: 'database.rules.json → missions/$missionId (327-328) + functions/awardMissionTokens.js (165)', remediation: 'Prohibir en el .validate de creación los campos de pago (tokensAwarded, awardedAmount, escrow).' },
      { id: 'PZ-020', priority: 20, severity: 'medium', necessity: 'alta', category: 'economy', title: 'CS2: cuenta partidas no jugadas juntas', description: 'verifyCs2FriendsMission solo exige que el contador total_matches_played de cada jugador suba en 1, sin correlacionar una partida común.', impact: 'Cobrar el premio de misiones CS2 jugando por separado, sin cumplir el objetivo de jugar en equipo.', location: 'functions/cs2FriendsMission.js → verifyCs2FriendsMission', remediation: 'Correlacionar una partida compartida real (código de lobby o replay) antes de aprobar el pago.' },
      { id: 'PZ-021', priority: 21, severity: 'medium', necessity: 'media', category: 'data', title: 'privateChats valida la sala por subcadena', description: 'La regla usa $chatRoomID.contains(auth.uid) en lugar de comparar los UID que componen la sala.', impact: 'Un identificador de sala manipulado que contenga el UID propio concede lectura y escritura fuera del par previsto.', location: 'database.rules.json → privateChats/$chatRoomID (560-565)', remediation: 'Descomponer el ID de sala y exigir que auth.uid sea uno de los dos participantes exactos.' },
      { id: 'PZ-022', priority: 22, severity: 'medium', necessity: 'media', category: 'storage', title: 'private_chat_images sin reglas de Storage', description: 'handlePrivateChatImageUpload sube a private_chat_images/{room}/ pero storage.rules no declara ninguna coincidencia para esa ruta.', impact: 'Las imágenes del chat privado fallan al subirse en producción y el flujo queda roto sin aviso.', location: 'playzone.js → handlePrivateChatImageUpload (3521) + storage.rules', remediation: 'Añadir match private_chat_images/{roomId}/{fileName} con auth, tope de tamaño y tipo MIME de imagen.' },
      { id: 'PZ-023', priority: 23, severity: 'medium', necessity: 'media', category: 'missions', title: 'Metadatos de participante sin validar', description: 'joinMission escribe nick, rank y photoURL elegidos por el cliente y las reglas de participants no imponen ningún esquema.', impact: 'Suplantar rango y nombre dentro del hub de la misión y colar URLs de avatar maliciosas.', location: 'playzone.js → joinMission (1725-1731) + database.rules.json → participants (329-332)', remediation: 'Validar longitud y formato, y tomar nick y rango del nodo users en el servidor.' },
