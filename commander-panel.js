(function() {
  ...
  // -------------------------------------------------
  // Calibrar cámara del personaje — rotación (yaw) + tamaño en pantalla,
  // guardados por personaje (no por clip) para que se apliquen a TODAS sus
  // animaciones en todo el sitio: overlay de nivel, torneos y transmisión en
  // vivo. Mientras se arrastran los controles solo se pisa en memoria vía
  // SGCreatureViewer.setCameraOverride() (ver welcome-overlay.js), así que la
  // vista previa se ve al instante sin ir y volver de Firebase; "Guardar"
  // recién en ese momento lo deja fijo para todo el mundo.
  // Reglas: nexusCharacterCamera/{characterId} en database.rules.json.
  // -------------------------------------------------
  ...
  function initCamCalibratePanel() {
    ...
    var savedForChar = null;   // último valor confirmado en Firebase para el personaje actual (null = de fábrica)
    var loadedBaseline = null; // lo que había en los controles justo al cargar/guardar/restablecer
    ...
    // El slider va de -180 a 180, pero un personaje puede tener su ángulo de
    // fábrica fuera de ese rango (el dragón usa 300°): sin esto el navegador
    // lo recorta a 180° apenas se carga, moviendo la cámara sin que nadie
    // haya tocado nada.
    ...
    /** Pisa el ángulo/tamaño en memoria y reencuadra la vista previa al instante. */
    function applyLive() {
      updateLabels();
